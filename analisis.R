...
natalidad_clean <- natalidad_raw %>%
  ...
  filter( nacionalidad == 'Ambas nacionalidades', !str_detect(ccaa, "Total Nacional")) %>%
...
paro_clean <- paro_raw %>%
  ...
  filter(sexo == "Mujeres", edad == "Total", !str_detect(ccaa, "Total Nacional"))%>%
  # Obtener el valor anual: 
  # Convertir valor a numero para que no salga NaN
  mutate(valor = str_trim(valor), # Quitamos los espacios en blanco
...
tabla_maestra_paro <- inner_join(paro_clean, natalidad_clean, by = c("anio", "ccaa")) 
...
correlacion_modelo1 <- cor(tabla_maestra_paro$tasa_paro, tabla_maestra_paro$tasa_natalidad, use = "complete.obs")
print(paste("La correlación de Pearson para el Modelo 1 (Tasa Paro y Tasa Natalidad) es:", round(correlacion_modelo1, 4)))
...
# JOIN de las tablas
tabla_maestra_mujeres <- inner_join(empleo_mujeres_clean, natalidad_clean, by = c("anio", "ccaa"))
...
ggplot(tabla_maestra_mujeres, aes(x = tasa_empleo, y = tasa_natalidad)) +
  ...
  labs(title = "Relación entre Tasa de Empleo Femenino y Natalidad",
       ...
       x = "Tasa de Empleo Femenino (%)", y = "Tasa de Natalidad (Nacimientos por 1.000 hab.)") +
  theme_minimal()

correlacion_modelo2 <- cor(tabla_maestra_mujeres$tasa_empleo, tabla_maestra_mujeres$tasa_natalidad, use = "complete.obs")
print(paste("La correlación de Pearson para el Modelo 2 (Tasa Empleo Femenino y Tasa Natalidad) es:", round(correlacion_modelo2, 4)))
...
empleo_hombres_clean <- empleo_raw %>%
  ...
  filter(sexo == "Hombres", edad == "Total", !str_detect(ccaa, "Total Nacional")) %>% 
  # Verificamos que los valores sean numéricos
  mutate(anio = as.numeric(anio), 
         tasa_empleo = as.numeric(str_replace(str_trim(tasa_empleo), ",", "."))) %>%
  # Eliminamos las columnas no necesarias
...
# JOIN de las tablas
tabla_maestra_hombres <- inner_join(empleo_hombres_clean, natalidad_clean, by = c("anio", "ccaa"))
...
ggplot(tabla_maestra_hombres, aes(x = tasa_empleo, y = tasa_natalidad)) +
  ...
  labs(title = "Relación entre Tasa de Empleo Masculino y Natalidad",
       subtitle = "Análisis por Comunidad Autónoma (2006-2024)",
       x = "Tasa de Empleo Masculino (%)", y = "Tasa de Natalidad (Nacimientos por 1.000 hab.)") +
  theme_minimal()

correlacion_modelo3 <- cor(tabla_maestra_hombres$tasa_empleo, tabla_maestra_hombres$tasa_natalidad, use = "complete.obs")
print(paste("La correlación de Pearson para el Modelo 3 (Tasa Empleo Masculino y Tasa Natalidad) es:", round(correlacion_modelo3, 4)))
